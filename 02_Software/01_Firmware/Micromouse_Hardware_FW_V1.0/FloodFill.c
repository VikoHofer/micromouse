...
#ifdef MOUSE_1_2		
		uint32_t const maxValue = 2300;
		uint32_t const maxValueFront = 1600;
...
void GetDriveCommands(uint8_t * mouseX, uint8_t * mouseY, Point shortestPath[gSizeMouseMaze * gSizeMouseMaze], 
	...
	for(int i = 0; i < fastPathIndex-1;i++){
		move(fastPathMove[i],2000,2000,0);
...
GetDriveCommands(&mouseX, &mouseY, shortestPath, &mouseDriveDirection,Index);
