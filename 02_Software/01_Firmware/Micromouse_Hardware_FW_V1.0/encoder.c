...
* -> 15 cycles for one round / 120 counts for one round
*******************************************************************************/
// TIM1 - ReadCounter Value
static int32_t Encoder_ReadCounter_Right(void)
{
	int32_t counter = -1*__HAL_TIM_GET_COUNTER(&htim1);
...
{
	int32_t counter = -1*__HAL_TIM_GET_COUNTER(&htim3);
	return (int32_t) counter;
}
...
/*******************************************************************************
* Get Position (120 Counts per Cycle)
*******************************************************************************/
static double position_right = 0.0;
static double position_left = 0.0;
static double position_right_reset_Turn = 0.0;
static double position_left_reset_Turn = 0.0;
...
static void Update_Encoder_Position_Right(void)
{
    ...
    position_right_reset_Turn += (double)delta;
...
static void Update_Encoder_Position_Left(void)
{
    ...
    position_left_reset_Turn += (double)delta;
...
//RHA wird für chain profiles benötigt
double Get_Encoder_Position_Raw_Right_reset_Turn(void)
{
    Update_Encoder_Position_Right();
    return (position_right_reset_Turn * -1);
}
double Get_Encoder_Position_Raw_Left_reset_Turn(void)
{
    Update_Encoder_Position_Left();
    return position_left_reset_Turn;
}
...
{
	return Get_Encoder_Position_Raw_Right() / COUNTS_PER_ROUND;
}
double Get_Encoder_Position_Rounds_Left(void)
{
	return Get_Encoder_Position_Raw_Left() / COUNTS_PER_ROUND;
}

double Get_Encoder_Position_Rounds_Right_reset_Turn(void)
{
	return Get_Encoder_Position_Raw_Right_reset_Turn() / 120.00;
}
double Get_Encoder_Position_Rounds_Left_reset_Turn(void)
{
	return Get_Encoder_Position_Raw_Left_reset_Turn() / 120.00;
}
...
{
	return Get_Encoder_Position_Rounds_Right_reset_Turn()*WHEEL_SIZE;
}
double Get_Encoder_Position_mm_Left_reset_Turn(void)
{
	return Get_Encoder_Position_Rounds_Left_reset_Turn()*WHEEL_SIZE;
}
...
*******************************************************************************/
volatile double fwd_change = 0.0;
volatile double rot_change = 0.0;

void Encoder_Update(void)
{
    static double pos_old = 0.0;
    static double ang_old = 0.0;

    double posL = Get_Encoder_Position_mm_Left();
    double posR = Get_Encoder_Position_mm_Right();

    double pos = 0.5 * (posL + posR);
    double ang = (posL - posR);           // mm-diff

    fwd_change = pos - pos_old;
    rot_change = ang - ang_old;
...
static int buffer_index_vel = 0; // Index f�r Ringpuffer

static float buffer_ang[FILTER_ORDER] = {0}; // Ringpuffer
static int buffer_index_ang = 0; // Index f�r Ringpuffer
...
float apply_fir_filter_velocity(float input) {
    ...
    for (int i = 0; i < FILTER_ORDER; i++) {
        ...
        index = (index - 1 + FILTER_ORDER) % FILTER_ORDER; // R�ckw�rts z�hlen, mit Wrap-Around
    }
...
void Encoder_Reset_reset_Turn(void){
    position_right_reset_Turn = 0.0;
    position_left_reset_Turn = 0.0;
}
