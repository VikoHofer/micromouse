...
HAL_StatusTypeDef BNO055_Init(BNO055_Handle_t* bno, I2C_HandleTypeDef* hi2c, uint8_t addr7)
{
    ...
    bno->mode = OPERATION_MODE_CONFIG; 
		...
		//uint32_t e = HAL_I2C_GetError(bno->hi2c);
	  //return e;
		if (s != HAL_OK) {
				uint32_t e = HAL_I2C_GetError(bno->hi2c); // put breakpoint or print e
				return s; // likely NACK if e has HAL_I2C_ERROR_AF
		}

		// 2) Force page 0 (optional safety)
		uint8_t page0 = 0;
		...
		// Basic bring-up: config mode, normal power, page 0, then NDOF
		uint8_t cfg = OPERATION_MODE_CONFIG;
		HAL_I2C_Mem_Write(bno->hi2c, bno->dev_address, BNO055_OPR_MODE_ADDR,
											I2C_MEMADD_SIZE_8BIT, &cfg, 1, bno->timeout);
		HAL_Delay(30);

		uint8_t pwr = POWER_MODE_NORMAL;
		HAL_I2C_Mem_Write(bno->hi2c, bno->dev_address, BNO055_PWR_MODE_ADDR,
											I2C_MEMADD_SIZE_8BIT, &pwr, 1, bno->timeout);
		HAL_Delay(10);

		HAL_I2C_Mem_Write(bno->hi2c, bno->dev_address, BNO055_PAGE_ID_ADDR,
											I2C_MEMADD_SIZE_8BIT, &page0, 1, bno->timeout);
...
HAL_StatusTypeDef BNO055_ReadEuler(BNO055_Handle_t* bno, bno055_vector_t* euler)
{
    ...
    HAL_StatusTypeDef s = HAL_I2C_Mem_Read(bno->hi2c, bno->dev_address,
                                           ...
                                           I2C_MEMADD_SIZE_8BIT, buf, 6, bno->timeout);
    if (s == HAL_OK) {
        euler->x = (int16_t)((buf[1] << 8) | buf[0]); // Heading
        euler->y = (int16_t)((buf[3] << 8) | buf[2]); // Roll
        euler->z = (int16_t)((buf[5] << 8) | buf[4]); // Pitch
    }
    return s;
} 
