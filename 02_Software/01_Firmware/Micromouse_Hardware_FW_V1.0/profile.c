...
//	}
	...
	//RHA Profile_Start neu schreiben und alle Werte manuell anpassbar machen
	//dann selber Werte f�r die folgenden Parameter setzen
	//profile->position = 0.0f;
	profile->position = profile->sign *profile->final_position;
...
//    float LOOP_INTERVAL = elapsedTime / 1000.0;
    ...
    if (profile->state != CS_FINISHED && remaining < 3.125f) {
        profile->state = CS_FINISHED;
