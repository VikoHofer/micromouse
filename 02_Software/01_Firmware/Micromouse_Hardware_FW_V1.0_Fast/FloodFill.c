...
//    }
    ...
    uint32_t const maxValue = MAX_VALUE_WALLS;
...
//			Motion_CommandForward(pathCommands[i].distance, vel_forward, acc_forward, vel_angle);
//		}
//		else{
//			Motion_CommandCurveAngle(pathCommands[i].distance, 90.0f, vel_angle, acc_curve, vel_angle);
//		}
//	}
//	Motion_Stop();
//}
...
//	float acc_curve = 999999.0f;
	...
	float vel_forward = 1200;
	float vel_angle = 700;

	float acc_forward = 4000.0f;
	float acc_curve = 999999.0f;
	uint8_t speedSetup = 0; //0 schnellster  Modus
	float speedCorrection = 1.0f;
	
	while(1){
		
		float offSetAfter_180Curve = 0; //Nach einer 180°Kurve etwas weiter fahren
		float const distanceOffset = 20.0f;
		...
		if(BUTTON1_Pressed()){
			...
			for(int i = 0; i < fastPathIndex;i++){
				if(pathCommands[i].isMove == true){
					//Bei kurzen Distanzen nicht schnell fahren damit Zeit zum regeln bleibt
					//Geschwindigkeit Kurven
					if(pathCommands[i].distance < 1.5f){
						//printf("pathCommands[i].distance: %f", pathCommands[i].distance);
						//Unterscheiden wie lange die geraden sind, je nachdem schneller fahren
						float driveDistance = pathCommands[i].distance*178.0f + offSetAfter_180Curve;
						offSetAfter_180Curve = 0;
						Motion_CommandForward(pathCommands[i].distance*178.0f, vel_angle, acc_forward, vel_angle, true);
					}
					//Geschwindigkeit 1500
					else if (pathCommands[i].distance > 5.0f){
						float driveDistance = pathCommands[i].distance*178.0f + offSetAfter_180Curve;
						offSetAfter_180Curve = 0;
						Motion_CommandForward(pathCommands[i].distance*178.0f, 1500, acc_forward, vel_angle, true);
					}
					// Standard Geschwindigkeit 1200
					else{
						//printf("pathCommands[i].distance: %f", pathCommands[i].distance);
						//Unterscheiden wie lange die geraden sind, je nachdem schneller fahren
						float driveDistance = pathCommands[i].distance*178.0f + offSetAfter_180Curve;
						offSetAfter_180Curve = 0;
						Motion_CommandForward(pathCommands[i].distance*178.0f, vel_forward, acc_forward, vel_angle, true);
					}
				}
				else{
					//Prüfung auf doppelt 180°
					if((i + 3) < fastPathIndex
						&& !pathCommands[i+1].isMove
						&& !pathCommands[i+2].isMove
						&& !pathCommands[i+3].isMove
						&& pathCommands[i].distance == pathCommands[i+1].distance
						&& pathCommands[i+2].distance == pathCommands[i+3].distance
						&& pathCommands[i].distance != pathCommands[i+2].distance)
					{
						//printf("180° Kurve doppelt\n");
						Motion_CommandCurveAngle((pathCommands[i].distance + pathCommands[i+1].distance)*0.85f, 85.0f, vel_angle, 999999.0f, vel_angle);
						Motion_CommandForward(85.0f, vel_angle, acc_forward, vel_angle, false);
						Motion_CommandCurveAngle((pathCommands[i+2].distance + pathCommands[i+3].distance)*0.85f, 85.0f, vel_angle, 999999.0f, vel_angle);
						i = i+3;
						offSetAfter_180Curve = 0*distanceOffset;
						continue;
					}
					if((i+1) < fastPathIndex && !pathCommands[i+1].isMove)
					{
							// einzelne 180°
							if(pathCommands[i].distance == pathCommands[i+1].distance)
							{
									//printf("180° Kurve\n");
									Motion_CommandCurveAngle((pathCommands[i].distance + pathCommands[i+1].distance),85.0f,vel_angle,999999.0f,vel_angle);
									i++;
									offSetAfter_180Curve = distanceOffset;
									continue;
							}
							...
							{
									uint8_t counter = 0;
									for(uint8_t j = i; j < fastPathIndex-1;j++){
										if(!pathCommands[j].isMove && !pathCommands[j+1].isMove && pathCommands[j].distance != pathCommands[j+1].distance){
											counter++;
										}
										else{
											break;
									...
									if(counter == 1){
										//printf("einmal S Kurven\n");
										Motion_CommandCurveAngle(pathCommands[i].distance * 0.684f,65.0f,vel_angle,999999.0f,vel_angle);
										Motion_CommandForward(125.0f,vel_angle,acc_forward,vel_angle,false);
										Motion_CommandCurveAngle(pathCommands[i+1].distance * 0.684f,65.0f,vel_angle,999999.0f,vel_angle);
										i = i + counter;
										offSetAfter_180Curve = 0 * distanceOffset;
									}
									//längere diagonale
										...
										int8_t sign = 0;
										uint8_t k;
										float curveValue = 0.685f;
										if(pathCommands[i].distance > 0){
											sign = 1;
										}
										else{
											sign = -1;
										...
										Motion_CommandCurveAngle(pathCommands[i].distance * curveValue,85.0f,vel_angle,999999.0f,vel_angle);
										for(k = 0; k < counter;k++){
											Motion_CommandForward(87.0f * speedCorrection,vel_angle,acc_forward,vel_angle,false);
											Motion_CommandCurveAngle(pathCommands[k+i+1].distance * (curveValue/2),85.0f,vel_angle,999999.0f,vel_angle);
											//Für die letze Kurve den Wert anpassen
											if(k == counter-2 && counter % 2 == 0){ //counter muss mindestens 2 sein daher -2 erlaubt
												curveValue = 2.0f*curveValue;
											}
											else if (k == counter-2 && counter % 2 == 1){
												curveValue = 2.0f*curveValue;
											}
										}
										i = i + counter;
										offSetAfter_180Curve = 0.5f * distanceOffset;
									}
					...
					//normale Kurve
					//printf("normale 90° Kurve\n");
					Motion_CommandCurveAngle(pathCommands[i].distance, 85.0f, vel_angle, acc_curve, vel_angle);
					offSetAfter_180Curve = 0*distanceOffset;
				}
			}
			//Ein Feld ausfahren in der Mitte
			Motion_CommandForward(175.0f, vel_angle, acc_forward, 0, true);
