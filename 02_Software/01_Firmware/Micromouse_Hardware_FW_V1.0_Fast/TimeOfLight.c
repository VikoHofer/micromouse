...
#ifndef TOF_45_INTERMEASUREMENT_MS
#define TOF_45_INTERMEASUREMENT_MS   11U
#endif
...
static uint16_t s_left45_mm  = VL6180X_INVALID_RANGE;
static uint16_t s_right45_mm = VL6180X_INVALID_RANGE;

static uint8_t s_left45_valid  = 0U;
static uint8_t s_right45_valid = 0U;

void TOF_Continuous45_Init(void)
{
    s_left45_mm = VL6180X_INVALID_RANGE;
    s_right45_mm = VL6180X_INVALID_RANGE;
    s_left45_valid = 0U;
    s_right45_valid = 0U;

    (void)VL6180X_StartContinuousAt(tof_sensor_addr[TOF_IDX_LEFT_45], TOF_45_INTERMEASUREMENT_MS);
    (void)VL6180X_StartContinuousAt(tof_sensor_addr[TOF_IDX_RIGHT_45], TOF_45_INTERMEASUREMENT_MS);
}
...
void TOF_Continuous45_Update(void)
{
    ...
    {
        s_left45_mm = mm;
        s_left45_valid = (mm != VL6180X_INVALID_RANGE) ? 1U : 0U;
    }

    if (VL6180X_ReadContinuousIfReadyAt(tof_sensor_addr[TOF_IDX_RIGHT_45], &mm) == TOF_OK)
    {
        s_right45_mm = mm;
        s_right45_valid = (mm != VL6180X_INVALID_RANGE) ? 1U : 0U;
    }
}
...
{
    return s_left45_valid;
}

uint8_t TOF_Right45HasValidData(void)
{
    return s_right45_valid;
}