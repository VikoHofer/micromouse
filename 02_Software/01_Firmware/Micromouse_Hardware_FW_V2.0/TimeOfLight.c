...
int VL6180X_InitMask(uint32_t active_mask)
{
    ...
    for (uint8_t i = 0; i < NUM_SENSORS; i++)
    {
        ...
        if (HAL_I2C_IsDeviceReady(&hi2c2,
                                  (uint16_t)(VL6180X_DEFAULT_ADDR << 1),
                                  5,
                                  100) != HAL_OK)
        {
            return -(10 + (int)i);
        ...
        VL6180X_Read8_at(VL6180X_DEFAULT_ADDR,
                         VL6180X_REG_IDENTIFICATION_MODEL_ID,
                         &model);
        ...
        if (VL6180X_WriteMandatorySettingsAt(VL6180X_DEFAULT_ADDR) != 0) {
            return -(40 + (int)i);
        }
        ...
        if (fresh == 1U) {
            VL6180X_Write8_at(VL6180X_DEFAULT_ADDR,
                              VL6180X_REG_SYSTEM_FRESH_OUT_OF_RESET,
                              0x00);
        }

        if (VL6180X_SetAddressAt(VL6180X_DEFAULT_ADDR,
                                 tof_sensor_addr[i]) != HAL_OK)
        {
            return -(50 + (int)i);
        }
    }
...
{
    return VL6180X_InitMask(TOF_ACTIVE_MASK);
}
/**
