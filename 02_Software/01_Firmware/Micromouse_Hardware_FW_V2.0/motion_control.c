...
#define WALL_FILTER_ALPHA           0   /* 0..1, h�her = ruhiger */
...
//			}
    ...
    if ((rm_old != WALL45_INVALID_MM) && (rm_new != WALL45_INVALID_MM) && (rm_new != rm_old)) {
        float remainingDistance = Profile_GetRemainingDistance(&forward_profile);

        if (remainingDistance < 135.0f) {
            if (rm_new > rm_old + 50U) {
                Profile_SetPosition(&forward_profile, 0.0f);
                Profile_SetFinalPosition(&forward_profile, 50.0f);
            }
        }
    ...
    if ((lm_old != WALL45_INVALID_MM) && (lm_new != WALL45_INVALID_MM) && (lm_new != lm_old)) {
        float remainingDistance = Profile_GetRemainingDistance(&forward_profile);

        if (remainingDistance < 135.0f) {
            if (lm_new > lm_old + 50U) {
                Profile_SetPosition(&forward_profile, 0.0f);
                Profile_SetFinalPosition(&forward_profile, 50.0f);
            }
        }
...
static float MotionController_CalcWallSteeringDps(void)
{
	if (WALL_STEER_ENABLE) {
		...
		/* Beide W�nde sichtbar -> Mittelwert der beiden normierten Fehler */
		if (left_valid && right_valid) {
		...
		/* Falls dein Motion-Timer mit LOOP_INTERVAL l�uft, hier besser LOOP_INTERVAL verwenden */
...
////Aktueller Winkel wird ber�cksichtigt
