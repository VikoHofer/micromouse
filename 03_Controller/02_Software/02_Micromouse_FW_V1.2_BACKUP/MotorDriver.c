...
void Motor_Init(void){
    ...
    tim4.Instance = TIM4;
    tim4.Init.Prescaler = 6; 
    tim4.Init.CounterMode = TIM_COUNTERMODE_UP;
    tim4.Init.Period = 255; 
	...
	// Start PWM channels
    HAL_TIM_PWM_Start(&tim4, TIM_CHANNEL_3);  // Motor Right
    HAL_TIM_PWM_Start(&tim4, TIM_CHANNEL_4);  // Motor Left
}
...
void Motor_Left_SetSpeed(uint16_t speed)
{
	...
	TIM_OC_InitTypeDef sConfigOC1;
    sConfigOC1.OCMode = TIM_OCMODE_PWM1;
    sConfigOC1.Pulse = speed; 
    sConfigOC1.OCPolarity = TIM_OCPOLARITY_HIGH;
    sConfigOC1.OCFastMode = TIM_OCFAST_DISABLE;
    HAL_TIM_PWM_ConfigChannel(&tim4, &sConfigOC1, TIM_CHANNEL_4);
	
	Motor_Left_PWM_Start();
}
void Motor_Right_SetSpeed(uint16_t speed)
{
	Motor_Right_PWM_Stop();
	
	TIM_OC_InitTypeDef sConfigOC2;
    sConfigOC2.OCMode = TIM_OCMODE_PWM1;
    sConfigOC2.Pulse = speed; 
    sConfigOC2.OCPolarity = TIM_OCPOLARITY_HIGH;
    sConfigOC2.OCFastMode = TIM_OCFAST_DISABLE;
    HAL_TIM_PWM_ConfigChannel(&tim4, &sConfigOC2, TIM_CHANNEL_3);
	
	Motor_Right_PWM_Start();
}
*/
