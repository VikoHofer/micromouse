...
{
	SensorID sensor45Left = SENSOR_ID_1;
	SensorID sensor45Right = SENSOR_ID_3;
	
	GPIO_Pin_t pin_sensor45Left = VCC_SENSOR_PIN[sensor45Left];
	GPIO_Pin_t pin_sensor45Right = VCC_SENSOR_PIN[sensor45Right];
	
	// turn on VCC_PIN 
    HAL_GPIO_WritePin(pin_sensor45Left.GPIO_Port, pin_sensor45Left.GPIO_Pin, GPIO_PIN_SET);
    while(HAL_GPIO_ReadPin(VCC_SENSOR_PIN[sensor45Left].GPIO_Port, VCC_SENSOR_PIN[sensor45Left].GPIO_Pin) != GPIO_PIN_SET);
...
void deactivateVCC_AngleSensors(void)
{
	...
	// turn on VCC_PIN 
    HAL_GPIO_WritePin(pin_sensor45Left.GPIO_Port, pin_sensor45Left.GPIO_Pin, GPIO_PIN_RESET);
    while(HAL_GPIO_ReadPin(VCC_SENSOR_PIN[sensor45Left].GPIO_Port, VCC_SENSOR_PIN[sensor45Left].GPIO_Pin) != GPIO_PIN_RESET);
...
Sensor_RetType Sensor_Read(SensorID sensor, ADC_HandleTypeDef* adcID, uint32_t* dist){
    ...
    while(HAL_GPIO_ReadPin(VCC_SENSOR_PIN[sensor].GPIO_Port, VCC_SENSOR_PIN[sensor].GPIO_Pin) != GPIO_PIN_SET);
    ...
    *dist = ADC_Convert(adcID, adc_channel);
    ...
    while(HAL_GPIO_ReadPin(VCC_SENSOR_PIN[sensor].GPIO_Port, VCC_SENSOR_PIN[sensor].GPIO_Pin) != GPIO_PIN_RESET);
...
//RHA wird nicht mehr verwendet? LÖSCHEN?
/**
